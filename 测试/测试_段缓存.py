...
IO 测试一律用 monkeypatch 把 H3_段缓存_DIR 指到 tmp_path，不污染真实 %TEMP%。
...
def _注入基(monkeypatch, tmp_path):
    """把 H3_段缓存_DIR 指到 tmp_path 下，返回**基**目录绝对路径（不含版本层）。

    只设环境变量、不触发 _缓存根()，以便调用方先在基下布置旧版本目录/散文件再验淘汰。
    """
    基 = str(tmp_path / "段缓存")
    monkeypatch.setenv("H3_段缓存_DIR", 基)
...
def test_淘汰旧版本_删旧世代但绝不碰无关内容(monkeypatch, tmp_path):
    ...
    的模型权重**——两个正则白名单是纵深防御，防 H3_段缓存_DIR 被误设到重要目录时误删。
