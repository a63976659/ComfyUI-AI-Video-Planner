# -*- coding: utf-8 -*-
"""Task 11 长视频规划师 V3 主节点：schema 冒烟 + I1/I2 契约回归锁。
...
节点长视频规划师 = pytest.importorskip("节点.长视频规划师")

from 节点.长视频规划师 import 长视频规划师  # noqa: E402


def test_V3_schema_冒烟():
    s = 长视频规划师.GET_SCHEMA()
    s.validate()   # V3 schema 完整性检查：任一 io.Schema 参数非法都会抛
    assert s.node_id == "长视频规划师"
...
def test_不覆写_validate_inputs_I1_lock():
    ...
    assert "validate_inputs" not in 长视频规划师.__dict__, \
        "长视频规划师 不得自定义 validate_inputs（含 **kwargs 会关闭全节点原生校验）"
...
def test_widget_边界走采样与解码常量_C1_lock():
    ...
    inputs = 长视频规划师.INPUT_TYPES()["required"]
...
def test_execute_空产物_抛ValueError_I2_lock(monkeypatch):
    ...
    with pytest.raises(ValueError, match="长视频规划师 无有效产物"):
        长视频规划师.execute(
...
def test_广播进度_发自有事件且失败吞掉(monkeypatch):
    ...
    节点长视频规划师._广播进度("n1", 3, 7)
    assert 发的 == [(节点长视频规划师._进度事件名, {"node": "n1", "value": 3, "max": 7}, "cid-1")], \
...
def test_execute_进度回调_宿主与自有双通道(monkeypatch):
    ...
    monkeypatch.setattr(节点长视频规划师, "ComfyAPISync", 假API)
    monkeypatch.setattr(假执行核心, "执行时间轴", 假执行时间轴)
    monkeypatch.setattr(假媒体路由, "媒体根", lambda: "/tmp")
    monkeypatch.setattr(节点长视频规划师, "_广播进度",
