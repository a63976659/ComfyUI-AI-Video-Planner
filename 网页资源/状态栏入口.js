...
app.registerExtension({
    ...
    async setup() {
        确保面板();
        // 段级进度走**自有**通道（后端 节点/导演台.py 的 _广播进度 发 _进度事件名），事件名是前后端
        // 字面量镜像，同步锁见 测试/测试_常量同步.py。
        // ⚠️ 这里**刻意不再监听**宿主的 legacy "progress"：那条事件由 KSampler 的内部去噪步进触发
        // （宿主 main.py 的 hijack_progress 全局 hook），其 node_id 取自执行上下文＝本导演台节点，
        ...
        api.addEventListener("h3_导演台_进度", (e) => {
            const d = e.detail || {};
            const 绑定 = 面板?.取节点();
            // 按绑定节点过滤：多节点工作流下只显示当前正在编辑的那个导演台的进度。
