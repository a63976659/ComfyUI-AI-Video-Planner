...
变量 长视频规划师_段缓存_DIR 覆盖）；h3-6 起**按 缓存格式版本 分子目录**存放，并由 _淘汰旧版本 在
...
# 把两种尺寸的段交给 流式拼接 混拼。
# h3-2 → h3-3：改双模型架构（长视频规划师 fl2va模型/ref2va模型 按段任务自动匹配）。采样参数移除了「模型
# 标识」维度——fl2va/ref2va 已由 指纹源 的 task 天然区分（task→槽确定），但同一槽上「换了哪个 ckpt
# 文件」不再入指纹；且移除该维度本身即改变指纹 payload。升版令 h3-3 前的旧缓存整体作废重算（干净
# 起点）。取舍：今后换 checkpoint 文件需手动清缓存目录（h3-6 起是 %TEMP%\长视频规划师_段缓存\<当前版本>，
# 长视频规划师_段缓存_DIR 覆盖的是**基**；⚠️ _淘汰旧版本 只删**旧版本**目录，同版本内不淘汰）
...
# 这是纵深防御——即使 长视频规划师_段缓存_DIR 被误设成某个重要目录，也绝不会碰到无关内容。
...
_日志 = logging.getLogger("长视频规划师.段缓存")
...
def _淘汰旧版本(基: str) -> None:
    ...
    是纵深防御：即使 长视频规划师_段缓存_DIR 被误设到重要目录，也只动本模块自己造过的东西。
...
def _缓存根() -> str:
    """当前版本的缓存目录（幂等创建）：<基>/<缓存格式版本>，基 默认 %TEMP%/长视频规划师_段缓存。

    环境变量 长视频规划师_段缓存_DIR 覆盖的是 **基**（不含版本层），测试注入 tmp_path 同理。
    ...
    """
    基 = os.environ.get("长视频规划师_段缓存_DIR") or os.path.join(
        tempfile.gettempdir(), "长视频规划师_段缓存"
    )
    基 = os.path.abspath(基)
